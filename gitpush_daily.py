...
def push_to_git(files_to_commit):
    ...
    try:
        ...
        subprocess.run(["git", "add", "."], check=True)  # Add all files in the repo
        ...
        # Commit the files
        commit_message = COMMIT_MESSAGE_TEMPLATE.format("all files")
        subprocess.run(["git", "commit", "-m", commit_message], check=True)
        
        # Push to the repository
        subprocess.run(["git", "push", "origin", BRANCH_NAME], check=True)
        print(f"Successfully pushed files to {BRANCH_NAME}.")
...
def main():
    ...
    if file_to_commit:
        ...
        # Optionally, commit and push the remaining files
        files = [f for f in os.listdir(FILES_DIR) if os.path.isfile(os.path.join(FILES_DIR, f))]
        files.remove(file_to_commit)  # Remove the file that was already pushed
        if files:
            push_to_git(files)
